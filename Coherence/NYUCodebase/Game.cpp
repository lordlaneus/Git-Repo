...
Game::Game()
{
	...
	addSprite("level", "resources\\Level Sheet.png", 0, 4, 2);
...
void Game::reset()
{
	...
	//resets level
	delete room;
	room = new Room(this, sprites["level"]);
	room->load("level1");
	//reset player
	delete player;
...
void Game::triggerParticles(ParticleEmitter& pe, Vector position, Tile t, float speed)
{
	pe.position = (position*3+t.position) / 4;
	pe.gravity = t.position;

	pe.velocity = position - t.position;
	pe.velocity.normalize(speed);

	pe.trigger();
}
void Game::triggerDust(Vector position, Tile t, float speed)
{
	dustEmitter.position = position;
	dustEmitter.gravity = t.position;

	dustEmitter.velocity = position - t.position;
	dustEmitter.velocity.normalize(speed);

	dustEmitter.trigger(&t);
}

void Game::update(float elapsed)
{
	room->update(elapsed);//meaningless; rooms don't change
